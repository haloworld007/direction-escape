...
### 15.1 关卡难度分级（基于阻挡深度系统）

> **核心设计理念**：难度由"阻挡深度"决定，而非简单的方块数量。
> 
> **阻挡深度定义**：消除一个方块前，需要先消除多少个阻挡它的方块。
> - 深度 0：直接可消（无阻挡）
> - 深度 1：被 1 个方块阻挡
> - 深度 N：需要解 N 层才能消除

| 阶段 | 关卡范围 | 方块数量 | 平均深度 | 最大深度 | 初始可消 | 特点 |
|------|----------|----------|----------|----------|----------|------|
| 成长期 | 1-15 | 100-140 | 0.5-0.8 | 2-3 | 30-35% | 单核心，浅层阻挡，需要基础规划 |
| 挑战期 | 16-35 | 140-175 | 0.8-1.2 | 3-4 | 22-28% | 核心更深，需要策略思考 |
| 大师期 | 36-60 | 175-200 | 1.2-1.6 | 4-5 | 16-22% | 双核心，需要在不同区域切换策略 |
| 传奇期 | 61+ | 200-220 | 1.6-2.0 | 5-6 | 12-16% | 多核心，复杂阻挡网络 |

> 方块数量会根据屏幕尺寸与布局容量动态微调。
> 每 5 关一个"休息关"，深度降低 20%，可消比例提高 8%。
...
- 衡量关卡整体难度的核心指标
- 数值越高，需要规划的消除路径越长
- 成长期约 0.5-0.8，传奇期可达 1.6-2.0

**最大阻挡深度**
- 关卡中"最难消除"方块需要解的层数
- 决定了关卡的"天花板"难度
- 成长期 2-3 层，传奇期可达 5-6 层

**初始可消比例**
- 开局时可直接消除的方块占比
- 比例越低，开局选择越有限，需要更精确的规划
- 成长期 30-35%，传奇期 12-16%
...
> 采用**程序化生成**策略，关卡通过**阻挡深度分布**来控制难度。
> 布局均为**菱形密排**结构，从中心向外扩展。
> 所有关卡 100% 程序生成，无硬编码布局。

**生成流程**

1. 生成方块位置（旋转网格布局）
2. 基于目标深度分配方向（核心区/中间区/边缘区）
3. 验证并调整以满足深度目标
4. 确保可解性（多轮模拟验证）
...
- **平均深度**：0.5-0.8
- **体验**：单核心结构，边缘可消比例高，需要基础规划但不会卡关

#### 挑战期（16-35关）

- **平均深度**：0.8-1.2
- **体验**：核心更深，需要思考"攻入核心"的路径

#### 大师期（36-60关）

- **平均深度**：1.2-1.6
- **体验**：双核心设计，需要在不同区域间切换策略
